...
class TrackingServientregaController extends Controller
{
    ...
    // Guardar TIFF como archivo para análisis manual
    ...
    // Probar diferentes decodificaciones del base64
    ...
    // MÉTODO CORREGIDO: Intentar conversión con Intervention Image
    private function intentarConversionInterventionImage($tiffBinario)
    {
        try {
            Log::info('🔄 Probando Intervention Image');

            // Intentar primero con Imagick si está disponible
            if (extension_loaded('imagick')) {
                try {
                    $manager = new ImageManager(new ImagickDriver());
                    $image = $manager->read($tiffBinario);

                    Log::info('✅ Intervention Image con Imagick - imagen cargada', [
                        'ancho' => $image->width(),
                        'alto' => $image->height()
                    ]);
                    ...
                    Log::warning('Intervention Image con Imagick falló: ' . $e->getMessage());
                }
            }
            ...
            try {
                ...
                Log::info('✅ Intervention Image con GD - imagen cargada', [
                    'ancho' => $image->width(),
                    'alto' => $image->height()
                ]);
                ...
                Log::error('Intervention Image con GD falló: ' . $e->getMessage());
            }

            return null;
        } catch (\Exception $e) {
            Log::error('❌ Intervention Image falló completamente: ' . $e->getMessage());
            return null;
        }
    }

    // Intentar conversión con GD nativo
    private function intentarConversionGD($tiffBinario)
    {
        try {
            Log::info('🔄 Probando GD nativo');
    ...
    // Intentar conversión con Imagick directo
    private function intentarConversionImagickDirecto($tiffBinario)
    {
        try {
            Log::info('🔄 Probando Imagick directo');

            if (!extension_loaded('imagick')) {
                Log::info('Imagick no disponible');
                return null;
            }
            ...
            Log::info('✅ Conversión con Imagick directo exitosa', [
                'tamaño_png' => strlen($pngData)
            ]);
            ...
            Log::error('❌ Imagick directo falló: ' . $e->getMessage());
            return null;
        }
    }
    ...
    private function convertirConAnalisisCompleto($tiffBinario, $analisis)
    {
        ...
        // Intentar conversión con Imagick directo primero
        $resultado = $this->intentarConversionImagickDirecto($tiffBinario);
        if ($resultado) {
            Log::info('✅ Conversión exitosa con Imagick directo');
            return $resultado;
        }
        ...
        // Intentar con GD como último recurso
    ...
    // CONVERSIÓN PRINCIPAL CON ANÁLISIS COMPLETO
    ...
    private function conversionAlternativa($tiffBase64)
    {
        try {
            Log::info('🔄 Probando conversión alternativa simplificada');
            ...
            if ($tiffBinario === false) {
                Log::error('❌ Decodificación base64 falló');
                return null;
            }

            Log::info('✅ Base64 decodificado', ['tamaño' => strlen($tiffBinario)]);

            // Verificar magic bytes
            $magicBytes = substr($tiffBinario, 0, 4);
            $isTiff = in_array(bin2hex($magicBytes), ['49492a00', '4d4d002a']);

            if (!$isTiff) {
                Log::warning('⚠️ No parece ser TIFF válido', ['magic' => bin2hex($magicBytes)]);
            }
            ...
            Log::error('❌ Error en conversión alternativa: ' . $e->getMessage());
            return null;
        }
    }
    ...
    private function procesarGuia($numeroGuia, $logAcceso = false)
    {
        ...
        // Procesar imagen con múltiples estrategias
        ...
        if (isset($array['Imagen']) && !empty($array['Imagen'])) {
            ...
            Log::info('=== INICIANDO CONVERSIÓN DE IMAGEN ===', [
                'longitud_original' => strlen($imagenBase64Original)
            ]);

            // Intentar conversión completa primero
            $imagenPngBase64 = $this->convertirTiffAPngConAnalisis($imagenBase64Original);

            // Si falla, usar método alternativo
            if (!$imagenPngBase64) {
                Log::warning('Conversión completa falló, probando método alternativo');
    ...
    public function debugImagen($numeroGuia)
    {
        try {
            ...
            if (isset($array['Imagen']) && !empty($array['Imagen'])) {
                ...
                $resultado = $this->conversionAlternativa($imagenLimpia);
    ...
    public function debugImagenCompleto($numeroGuia)
    {
        try {
            ...
            $resultado = $this->conversionAlternativa($imagenOriginal);
    ...
    // MÉTODO DE DIAGNÓSTICO DEL SISTEMA
    public function diagnosticoSistema()
    {
        ...
        if (extension_loaded('imagick')) {
            try {
                $imagick = new \Imagick();
                $diagnostico['imagick_version'] = $imagick->getVersion();
                $diagnostico['imagick_formats'] = $imagick->queryFormats();
                $diagnostico['imagick_supports_tiff'] = in_array('TIFF', $imagick->queryFormats());
            } catch (\Exception $e) {
                $diagnostico['imagick_error'] = $e->getMessage();
            }
        }

        if (extension_loaded('gd')) {
            $diagnostico['gd_info'] = gd_info();
        }

        Log::info('Diagnóstico del sistema:', $diagnostico);

        return response()->json($diagnostico);
    }

    // MÉTODO DE PRUEBA SIMPLIFICADO
    public function testConversionSimple($numeroGuia)
    {
        try {
            Log::info('=== INICIANDO TEST DE CONVERSIÓN SIMPLE ===', ['guia' => $numeroGuia]);
            ...
            if (!isset($array['Imagen']) || empty($array['Imagen'])) {
                return response()->json(['error' => 'No hay imagen en la respuesta']);
            }

            $imagenOriginal = is_array($array['Imagen']) ? $array['Imagen'][0] : $array['Imagen'];

            Log::info('Imagen encontrada', ['longitud' => strlen($imagenOriginal)]);

            // Usar el método alternativo corregido
            $resultado = $this->conversionAlternativa($imagenOriginal);
            ...
            if ($resultado) {
                ...
                $trackingRecord = TrackingServientrega::updateOrCreate(
                    ...
                    ['imagen_base64' => $resultado]
                );

                Log::info('✅ Imagen convertida y guardada', [
                    'longitud_resultado' => strlen($resultado),
                    'id_registro' => $trackingRecord->id
                ]);

                return response()->json([
                    'success' => true,
                    'mensaje' => 'Conversión exitosa y guardada en BD',
                    'longitud' => strlen($resultado),
                    'preview' => 'data:image/png;base64,' . substr($resultado, 0, 100) . '...'
                ]);
            }

            return response()->json([
                'error' => 'Conversión falló - revisa los logs para más detalles'
            ]);
        } catch (\Exception $e) {
            Log::error('❌ Error en test de conversión: ' . $e->getMessage());
            return response()->json(['error' => $e->getMessage()]);
        }
    }

    // MÉTODO PARA FORZAR REPROCESAMIENTO DE IMAGEN
    public function reprocesarImagen($numeroGuia)
    {
        try {
            Log::info('=== REPROCESANDO IMAGEN ===', ['guia' => $numeroGuia]);

            // Buscar registro existente
            $trackingRecord = TrackingServientrega::where('numero_guia', $numeroGuia)->first();

            if (!$trackingRecord) {
                return response()->json(['error' => 'Guía no encontrada en BD']);
            }
            ...
            if (!isset($array['Imagen']) || empty($array['Imagen'])) {
                return response()->json(['error' => 'No hay imagen en la API']);
            }

            $imagenOriginal = is_array($array['Imagen']) ? $array['Imagen'][0] : $array['Imagen'];

            // Forzar nueva conversión
            $resultado = $this->conversionAlternativa($imagenOriginal);
            ...
            return response()->json(['error' => 'Reprocesamiento falló']);
        } catch (\Exception $e) {
            Log::error('Error en reprocesamiento: ' . $e->getMessage());
            return response()->json(['error' => $e->getMessage()]);
        }
    }
}
